...
import { APIPromise } from '@blockaid/client/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['BLOCKAID_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Blockaid) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['BLOCKAID_LOG'] = 'debug';
      const client = new Blockaid({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['BLOCKAID_LOG'] = 'not a log level';
      const client = new Blockaid({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'BLOCKAID_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    test('in request options overridden by client options', () => {
      const client = new Blockaid({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['BLOCKAID_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Blockaid({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
