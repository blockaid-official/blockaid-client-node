...
export class Address extends APIResource {
   ...
   * this address is malicious or not as well as textual reasons of why the address
   * was flagged that way.
   */
  scan(body: AddressScanParams, options?: Core.RequestOptions): Core.APIPromise<AddressScanResponse> {
    return this._client.post('/evm/address/scan', { body, ...options });
  }
}

export interface AddressScanResponse {
  simulation?: AddressScanResponse.TransactionSimulation | AddressScanResponse.TransactionSimulationError;

  validation?: AddressScanResponse.TransactionValidation | AddressScanResponse.TransactrionValidationError;
}
